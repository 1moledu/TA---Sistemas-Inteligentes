...
df = pd.read_csv(r'datasets\data_408v_94x94\env_vital_signals_cego.txt', header = None)

indices_desejados = [3, 4, 5]
df_selecionado = df.iloc[:, indices_desejados]

df_selecionado.columns = ["qPA", "Pulso", "frequência Respiratória"]
...
clf = joblib.load(r'modelo_arvore_decisao.pkl')


previsoes = clf.predict(df_selecionado)

salvar = df.iloc[:, 0 , 1, 2, 3, 4]
